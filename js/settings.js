...
(function () {
    ...
    function renderIntro(root) {
        ...
        box.appendChild(el('p', { text: t('saml_provider',
            'It lets people log in to other services (like Kimai, GitLab or Grafana) with their Nextcloud account. ' +
            'Nextcloud acts as the "identity provider" (IdP) - the central place that checks passwords. ' +
            'Each external service is a "service provider" (SP) that trusts Nextcloud to confirm who someone is.') }));
    ...
    function renderCert(root) {
        ...
        section.appendChild(el('p', { 'class': 'saml-provider-note', text: t('saml_provider',
            ...
            'Copy the single-line version into services that ask for an "x509 certificate" (e.g. Kimai).') }));
    ...
    function renderSpRow(sp, tbody) {
        ...
        var details = el('details', {}, [
            ...
            el('p', {}, [el('strong', { text: t('saml_provider', 'Additional attributes (advanced, JSON)') }), el('br'),
                mappingArea, el('br'),
                el('small', { 'class': 'saml-provider-note', text: t('saml_provider',
                    'Username, display name and e-mail are always sent. Here you can add more names the service expects, ' +
                    'e.g. {"groups": "uid"}. Available values: uid = username, displayName = full name, mail = e-mail.') })]),
    ...
    function renderSps(root) {
        ...
        section.appendChild(el('p', { 'class': 'saml-provider-note', text: t('saml_provider',
            'You need two things from the services SAML settings: its "Entity ID" (its unique name) and its "ACS URL" (login callback address). ' +
            'For example Kimai: Entity ID = https://kimai.example.com/auth/saml/metadata, ACS URL = https://kimai.example.com/auth/saml/acs') }));
