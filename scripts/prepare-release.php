...
if ($argc !== 5) { fwrite(STDERR, "Usage: prepare-release version min max php-json\n"); exit(2); }
[, $version, $min, $max, $phpJson] = $argv;
$php = json_decode($phpJson, true);
if (!preg_match('/^\d+\.\d+\.\d+$/', $version) || !ctype_digit($min) || !ctype_digit($max) || (int)$min > (int)$max || !is_array($php) || $php === []) { fwrite(STDERR, "Invalid compatibility metadata.\n"); exit(2); }
