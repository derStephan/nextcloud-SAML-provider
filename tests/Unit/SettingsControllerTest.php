...
final class SettingsControllerTest extends TestCase {
 ...
 public function testUpdatesValidProviderAndNormalizesEmptyOptionalValues():void { $sp=new ServiceProvider(); $sp->setSpEntityId('https://sp.test/meta'); $sp->setAcsUrl('https://sp.test/acs'); $sp->setNameIdFormat('urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress'); $sp->setAttributeMapping('{}'); $this->mapper->byId=$sp; $r=$this->controller->updateSp(1,['spName'=>'Updated','sloUrl'=>'','attributeMapping'=>'{}','isEnabled'=>false]); self::assertSame(200,$r->status); self::assertSame('Updated',$r->data['spName']); self::assertFalse($r->data['isEnabled']); }
 public function testSavesOrganizationAndGeneratesCertificate():void { $config=new AppConfig(); $idp=new IdpConfigService($config,new UrlGenerator()); $controller=new SettingsController('saml_provider',new Request(),$this->mapper,$idp,new L10N(),new NullLogger()); self::assertSame('Example',$controller->saveIdp('Example')->data['orgName']); $generated=$controller->generateCert(); self::assertSame(200,$generated->status); self::assertNotSame('',$generated->data['certificate']); }
